...
## What’s inside
- LangGraph agent with nodes: query guard → policy retrieval + memory prep → merger (LLM) → compliance check → sentiment tag; refusal path for transactional/PII.
...
- Structured logging: JSONL at `logs/banking_rag_conversations.jsonl`; offline eval script in `scripts/eval_logs.py`.
...
- Optional debug:  
  ...
  - Compliance off: `BANK_RAG_ENABLE_COMPLIANCE_CHECK=0 …`
  - Sentiment debug: `BANK_RAG_DEBUG_SENTIMENT=1 …`
  - Compliance debug: `BANK_RAG_DEBUG_COMPLIANCE=1 …`
  - Logging off: `BANK_RAG_ENABLE_LOGGING=0 …`
...
- Logs go to `logs/banking_rag_conversations.jsonl` (one JSON per turn). Analyze via `python3 scripts/eval_logs.py`.
